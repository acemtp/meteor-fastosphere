...
semverExtract = string => {
  const re = /(\d+\.\d+(?:\.\d+)*)(?:-[a-zA-Z0-9-\.]*)?(?:_[0-9]*)?(?:\+[a-zA-Z0-9-]*)?/gm;
  const m = re.exec(string);
  let sv;
  if (m) {
    sv = m[0];
    if (sv.split('.').length === 2) sv += '.0';
  }
  return sv;
};
...
semverCompare = (versionOne, versionTwo) => {
  const meteorVOne = extractSemverPart(versionOne);
  const meteorVTwo = extractSemverPart(versionTwo);
